...
for i in range(2):
    for i in range(2):
        ...
        if abs(int(z/2)-10) >= 20:
            tello.go_xyz_speed(int(z/2)-10,0,0,10)
            time.sleep(1)
        
        elif int(z/2)-10 > 0:
            tello.go_xyz_speed(-20,0,0,10)
            tello.go_xyz_speed(20+int(z/2)-10,0,0,10)
            time.sleep(1)
    ...
    y = poseDict['y']
    z = poseDict['z']
    center = poseDict['center']
    w = poseDict['w']
    
    while abs(center[0]-int(w/2)) >= 60:
        poseDict = pose_estimation()
        beta = poseDict['beta']
        x = poseDict['x']
        y = poseDict['y']
    ...
    """
    if abs(-2*x/3)>=20:
        tello.go_xyz_speed(0,int(-2*x/3),0,10)
        time.sleep(1)
        
        
    elif int(-2*x/3)<=0:
        tello.go_xyz_speed(0,20,0,10)
        tello.go_xyz_speed(0,-20+int(-2*x/3),0,10)
        time.sleep(1)
